...
future_heatmap = np.ones((80,181))

for i in np.arange(71,151,1):
    lat_bin_sum = lat_bin_sum * (1 + growthrates[i,1]/100) # sum rows and sum columns. Unit is people
    long_bin_sum = GPW4refined.sum(axis=0)
    longitude_bins = len(long_bin_sum)
    latitude_bins = len(lat_bin_sum)

    print('Sense check 1: World Population for year', 1950+i,'is', int(sum(lat_bin_sum)/1e6), 'million people')
    ...
    future_heatmap[i-71] = critical_cas_area

np.savetxt("2021-2100", future_heatmap)
